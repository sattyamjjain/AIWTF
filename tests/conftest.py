...
def get_settings_override():
    ...
    return Settings(
        OPENAI_API_KEY="test-openai-key", SERPAPI_API_KEY="test-serp-key", TESTING=True
    )
...
def mock_research_service(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.api.routes.research.ResearchService", MagicMock(return_value=service)
    )
...
def MOCK_RESEARCH_REQUEST():
    return {"topic": "Test Research Topic", "depth": 1, "max_sources": 5}
